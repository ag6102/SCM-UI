...
export class TimetableComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let parsed = parser.parseFromString(xmlString, "application/xhtml+xml")
    console.log(parsed);
    ...
    for (let i = 0; i < allStations.length; i++){
      ...
      if (stationName[0].textContent.toLowerCase().trim() === this.data.name.toLowerCase().trim())
      {
  ...
  extractInfoFromTargetStation() {
    if (this.targetStation && this.targetStation.length > 0) {
      this.targetStation = this.targetStation.map(node => {
        return {
